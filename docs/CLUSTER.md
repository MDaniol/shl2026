...
├── env.sh               # ← source in terminals/batch: env vars + activates your own env
├── venv/                # lead-only reference env (seeds cache, builds container)
├── repo/                # canonical clone + the shared uv.lock (lead-managed)
